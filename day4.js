...
Count the number of valid passports - those that have all required fields. Treat cid as optional. In your batch file, how many passports are valid? correct answer is 216
...
Count the number of valid passports - those that have all required fields and valid values. Continue to treat cid as optional. In your batch file, how many passports are valid? correct answer is 150
*/
...
function validate(data) {
  let count = 0;
  data.forEach(({byr, iyr, eyr, hgt, hcl, ecl, pid}) => {
    if(isBirthYearValid(Number(byr)) &&
      isIssueYearValid(Number(iyr)) &&
      isExpirationYearValid(Number(eyr)) &&
      isHeightValid(hgt) &&
...
function isIssueYearValid(year) {
  return year >= 2010 && year <= 2020
}

function isExpirationYearValid(year) {
  return year >= 2020 && year <= 2030
}

function isHeightValid(height) {
  const unit = height.slice(height.length - 2)
  const size = Number(height.slice(0, height.length - 2))
  if(unit === 'cm') {
